...
namespace Alidu.Core.MessageBus.InMemory
{
    class InMemoryMessageBusSubscriptionsManager : IMessageBusSubscriptionsManager
    {
        ...
        public IEnumerable<SubscriptionInfo> GetHandlersForMessage<T>() where T : IntegrationMessage
        {
        ...
        public bool HasSubscriptionsForMessage<T>() where T : IntegrationMessage
        {
            var key = GetMessageKey<T>();
